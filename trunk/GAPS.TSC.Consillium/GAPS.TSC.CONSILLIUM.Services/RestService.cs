...
namespace GAPS.TSC.CONSILLIUM.Services
{
    public static class RestService
    {
        ...
        public static T Get<T>(string url, IEnumerable<KeyValuePair<string, string>> data = null) where T : new()
        {
        ...
        public static bool Post<T>(string url, IEnumerable<KeyValuePair<string, string>> data, out T output) where T : new()
        {
            var client = new RestClient(ApiBaseUrl);
            var req = new RestRequest(url, Method.POST);
            req.AddQueryParameter("key", ApiKey);
